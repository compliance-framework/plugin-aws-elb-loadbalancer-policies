...
| `compliance_framework.elbv2_ownership_tags` | Flags load balancers missing a non-empty owner tag. | `ACM_TLS_ENDPOINTS` | `ctrl-cc6-7-017` |
...
| `compliance_framework.elbv2_asset_disposal` | Optionally verifies failed or inactive load balancers have deletion audit evidence. | `ACM_TLS_ENDPOINTS` | `ctrl-cc6-5-001`, `ctrl-cc6-7-003`, `ctrl-cc6-7-006` |
| `compliance_framework.elbv2_management_change_audit_events` | Optionally verifies listener/rule management changes are present, attributable, and timely. | `ACM_TLS_ENDPOINTS` | `ctrl-cc6-2-015`, `ctrl-cc6-2-019`, `ctrl-cc6-3-013`, `ctrl-cc6-7-018` |
...
| `data.require_disposal_audit_events` | `false` | When `true`, inactive or failed load balancers must have a matching `Delete*` CloudTrail event. |
