...
test_failed_state_with_listener_delete_event_violation if {
	inp := {
		"resource": {"type": "loadbalancer", "id": "lb-1"},
		"config": {"state": "failed"},
		"dynamic": {"cloudtrail_events": [{"event_name": "DeleteListener"}]},
	}
	data.compliance_framework.elbv2_asset_disposal.violation[{"id": "disposal_process_unverified"}] with input as inp with data.require_disposal_audit_events as true
}

test_failed_state_with_rule_delete_event_violation if {
	inp := {
		"resource": {"type": "loadbalancer", "id": "lb-1"},
		"config": {"state": "failed"},
		"dynamic": {"cloudtrail_events": [{"event_name": "DeleteRule"}]},
	}
	data.compliance_framework.elbv2_asset_disposal.violation[{"id": "disposal_process_unverified"}] with input as inp with data.require_disposal_audit_events as true
}

test_failed_state_with_configured_disposal_delete_event_ok if {
	inp := {
		"resource": {"type": "loadbalancer", "id": "lb-1"},
		"config": {"state": "failed"},
		"dynamic": {"cloudtrail_events": [{"event_name": "RemoveLoadBalancer"}]},
	}
	count(data.compliance_framework.elbv2_asset_disposal.violation) == 0 with input as inp with data.require_disposal_audit_events as true with data.disposal_delete_event_names as ["RemoveLoadBalancer"]
}
